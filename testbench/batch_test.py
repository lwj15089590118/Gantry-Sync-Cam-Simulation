...
5. 看板参数上限联锁可达演示：失配 90%（看板滑动条/接口夹取上限）、
   补偿关、默认阈值 10mm 即触发同步联锁并验证动作链 ——
   原 60% 上限稳态 Δ≈7.5mm < 10mm，web 端永远无法演示"超差即停"。
...
def run_self_checks(
    ...
    # （原 60% 上限 Δ≈7.5mm < 默认阈值 10mm，永远触发不了）
