...
$$\Delta = e_1 - e_2 = \frac{v}{K_{p1}} - \frac{v}{K_{p2}}
...
任一周期检测到 $|\Delta|$ > 阈值 → 双轴立即停机并锁存报警。
演示工况（30% 失配、阈值 2mm）：无补偿触发停机，有补偿正常完成
（仿真验证值，可复现命令见 README）。
...
python 启动看板.bat               # 或 python dashboard/app.py → http://127.0.0.1:5000
```
